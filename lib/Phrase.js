...
class Phrase {
	...
	_isTag(type) {
		return !!Phrase.tag && type in Phrase.tag;
	}

	_isAlias(type) {
		return !!Phrase.alias && type in Phrase.alias;
	}
	...
	toJSON() {
		return {
			index: [this.words[0].index + 1, this.words[1].index + 1],
			type: this.type,
